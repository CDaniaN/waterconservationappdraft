    ...
    <script>
      ...
      waterUsageForm.addEventListener('submit', async (event) => {
        ...
        if (error) {
          ...
          console.log('Water usage data was submitted successfully!');
        }
      });

      //retrieving the data from the database
      const { data, error } = await supabase.from('water_usage').select('total_usage').order('id', { ascending: false }).limit(2);
